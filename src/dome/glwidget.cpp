...
// restore view definition after window reshape
  ...
  GLdouble eyeX=10; //pointCloud.boundingBox().size().length()*cos(angle/100.0);
	GLdouble eyeY=10; //pointCloud.boundingBox().size().y*1.5;
	GLdouble eyeZ=10; //pointCloud.boundingBox().size().length()*sin(angle/100.0); 
	GLdouble upX=0;
	GLdouble upY=1;
	GLdouble upZ=0;
...
void GLWidget::paintGL()
{
    ...
    gex::Vec3 c; // = 0.5*(pointCloud.boundingBox().max.vec3f() + pointCloud.boundingBox().min.vec3f());
