...
void CalcPixelRandom()
{
	...
	vec4 s1 = sin(time * 100000.422 + gl_FragCoord.xxxx * vec4(324.324234, 563.324234, 657.324234, 764.324234)) * 543.3423;
	vec4 s2 = sin(time * 100000.3422 + gl_FragCoord.yyyy * vec4(567.324234, 435.324234, 432.324234, 657.324234)) * 654.5423;
	gPixelRandom = fract(210042.4 + s1 + s2);
...
{          
	vec4 vResult = vec4(20.0, -1.0, 0.0, 0.0);
       
	float oP=length(vPos);
	vec3 vSphereDomain=vPos;
	float tt=time*0.7+40.0;
	vSphereDomain.x=sin(vSphereDomain.x)+sin(tt * 0.01);
	vSphereDomain.z=sin(vSphereDomain.z)+sin(tt * 0.01);

	vec4 vDistSphere = vec4( length((vSphereDomain))-0.2-sin(oP -tt*2.0), 2.0, vSphereDomain.xy);
       
	vResult = vDistSphere;
		       
	vec4 vDistFloor = vec4(vPos.y + 4.0, 1.0, vPos.xz);
...
vec3 GetLightPos()
{
	vec3 vLightPos = vec3(mouse.x*10.0, mouse.y*9.0, mouse.y*2.0);
	#ifdef ENABLE_MONTE_CARLO        
...
vec3 GetLightCol()
{
	return vec3(24.0, 5.0, 1.0) * 100.0;
}
...
void main( void )
{
	...
	const float fCamreaInitialHeading = 0.05;
	const float fCamreaInitialElevation = -1.5;
	const float fCamreaInitialDist = 5.0;
  ...
  if (fisheye_direction(gl_FragCoord.xy,resolution,0.5*PI,time*0.1,0.0,ray.vDir) < 0.0)
  {
