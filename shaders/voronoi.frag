...
void main( void ) 
{
	...
	uv = uv * mouse.x - mouse.x / 2.0;
	uv.x *= resolution.x / resolution.y;


	// ray origin
	vec3 ro = vec3(-40.0, -40.0, time * 0.0 - 5.0);
	ro.y = 0.0;
	// camera look at
	vec3 ta = vec3(2.0, 52.0,2.0);
...
//	vec3 rd = normalize(uv.x * uu + uv.y * vv + 1.0 * ww);
	vec3 rd;
  if (fisheye_direction(gl_FragCoord.xy,resolution,PI/2.0,0,0,rd) < 0.0)
	{
	...
	{
		float layer = float(i);
		its = intersect(ro, rd, vec3(0.0, -1.0 - layer * 2.0, 0.0), vec3(3.0, 0.0, 0.0), vec3(0.0, 0.0, 1.0));
		if(its.x > 0.0)
		{
	...
	vec3 col = pow(vec3(inten, (inten * 0.5), inten), 0.5 * vec3(0.15, 1.7, 9.0)); //pow(base color, glow amount)
