...
float map(vec3 p)
{
	...
	vec3 IP = vec3(-.011*cos(t+13.0),0.22*cos(t+27.0),2.0*cos(t*0.2+3.0));
	vec3 DP = p-IP;
...
void main( void )
{
    vec3 camPos = vec3(cos(time*0.2)*0.5, sin(time*0.2)*0.5, 1.0135);
    ...
    if (fisheye_direction(gl_FragCoord.xy,resolution,0.5*PI,time*0.1,0.0,rayDir) < 0.0)
    {
