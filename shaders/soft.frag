...
vec4 map( vec3 p )
{
    ...
    // pattern
	float f;
	vec3 q = p*0.85                     - vec3(0.0,1.0,0.0)*time*0.302;
    f  = 0.60000*noise( q ); q = q*2.02 - vec3(0.0,1.0,0.0)*time*0.02;
    f += 0.65000*noise( q ); q = q*2.03 - vec3(0.0,1.0,0.0)*time*0.32;
...
vec3 raymarch( in vec3 ro, in vec3 rd )
{
	...
	for( int i=0; i<25; i++ )
	{
		...
		col.xyz = mix( bg, col.xyz, exp(-0.004*t*t*t) );
...
void main(void)
{
  ...
  vec3 rd;
  if (fisheye_direction(gl_FragCoord.xy,resolution,time,0.5*time,-0.25*time,rd) < 0.0) 
  {
